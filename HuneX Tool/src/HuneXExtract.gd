				...
				while !start_off == 0x65DC8:
					...
					if test_name == 0x6E696231: #lbin
						...
						if files in range(1190, 10361):
							...
							file_extension = ".pss"
						elif files == 4 or files in range(1147, 1189):
							file_extension = ".ltex"
							if files in range(1147, 1157) or files in range(1160, 1167): #not sure how to handle the other images
								var image_type:int = 1
								var has_palette:bool = true
								var bits_per_color:int = 32
								var bpp:int = 8
								var width:int = mem_file.decode_u16(2)
								var height:int = mem_file.decode_u16(4)
								var tga_header:PackedByteArray = makeTGAHeader(has_palette, image_type, bits_per_color, bpp, width, height)
								
								final_image.append_array(tga_header)
								tga_header.clear()
								
								var image:PackedByteArray = mem_file.slice(mem_file.size() - 0x7C0, - 0x3C0) #some odd data after the palette
								image = unswizzle_palette(image)
								...
								image = mem_file.slice(0x20, mem_file.size() - 0x7E0) #image data
								final_image.append_array(image)
								image.clear()
								#mem_file.clear()
